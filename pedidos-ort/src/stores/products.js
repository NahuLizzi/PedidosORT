...
const state = reactive({
  products: [
    ...
    { id: 'B002', name: 'Agua con gas', price: 1300, img: '/img/agua_gas.png', category: 'Bebidas' },
...
export function useProductsStore() {
  const all = computed(() => state.products)
  const byCategory = category => state.products.filter(p => p.category === category)
  const byId = id => state.products.find(p => p.id === id)
  return { all, byCategory, byId }
}
